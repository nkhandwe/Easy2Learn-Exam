<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">{{ __('Concept') }}</h4>
        </template>
        <template #actions>
            <inertia-link :href="route('lessons.create')" class="qt-btn qt-btn-success">
                {{ __('New') }} {{ __('Concept') }}
            </inertia-link>
        </template>
            ...
            <div class="card">
                <div class="card-body">
                    <vue-good-table mode="remote" @on-page-change="onPageChange" @on-column-filter="onColumnFilter" @on-per-page-change="onPerPageChange"
                                    ...
                                    :totalRows="lessons.meta.pagination.total"
                                    :rows="lessons.data" :rtl="$page.props.rtl">
                        <template slot="table-row" slot-scope="props">
                            ...
                            <div v-else-if="props.column.field === 'actions'">
                                <actions-dropdown>
                                    <template #actions>
                                        <button @click="editLesson(props.row.id)" class="action-item">{{ __('Edit') }}</button>
                                        <button @click="deleteLesson(props.row.id)" class="action-item">{{ __('Delete') }}</button>
                                    </template>
                                </actions-dropdown>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editLesson(id) {
                this.$inertia.get(route('lessons.edit', {lesson: id}));
            },
            deleteLesson(id) {
                this.$confirm.require({
                    ...
                    accept: () => {
                        this.$inertia.delete(route('lessons.destroy', {id: id}), {}, {
                            onSuccess: () => {
                                this.$toast.add({
                                    severity: 'info',
