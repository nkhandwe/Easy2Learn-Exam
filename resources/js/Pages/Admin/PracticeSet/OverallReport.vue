<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">{{ practiceSet.title }} - {{ __('Overall Report') }}</h4>
        </template>
        <template #actions>
            <inertia-link :href="route('practice-sets.detailed_report', { practice_set: practiceSet.id })" class="qt-btn qt-btn-success">
                {{ __('Detailed Report') }}
            </inertia-link>
        </template>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-white shadow rounded-lg w-full overflow-hidden">
                    <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        ...
                        <admin-test-stat-card :title="__('Unique Attempt')" :count="stats.unique_test_takers"></admin-test-stat-card>
