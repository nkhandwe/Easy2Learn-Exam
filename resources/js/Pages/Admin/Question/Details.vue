        ...
        <div class="container mx-auto pt-4 px-4 sm:px-6 lg:px-8">
            <div class="w-full bg-white dark:bg-gray-800 py-5 flex flex-col xl:flex-row items-start xl:items-center justify-between px-5 xl:px-10 shadow rounded-t">
                <div class="mb-4 sm:mb-0 md:mb-0 lg:mb-0 xl:mb-0 lg:w-1/2">
                    <h2 class="text-gray-800 dark:text-gray-100 text-lg font-bold">{{ __('Question') }} {{ __('Details') }}</h2>
                        ...
                        <div class="xl:w-8/12 w-full py-2 md:pb-0 md:px-6">
                            <form @submit.prevent="submitForm">
                                ...
                                <div class="w-full flex flex-col mb-6">
                                    <label for="question" class="pb-2 font-semibold text-gray-800 text-sm">{{ __('Question') }}</label>
                                    <ckeditor id="question" :editor-url="editorUrl" v-model="form.question" :config="editorConfig" :class="[$page.props.rtl ? 'cke_rtl' : 'cke_ltr', errors.question ? 'p-invalid' : '']"></ckeditor>
                                    <div class="form-control-errors">
                                        <p v-if="$v.form.question.$error && !$v.form.question.required" role="alert" class="text-xs text-red-500 pt-2">{{ __('Question') }} {{ __('is required') }}</p>
                                    </div>
                                </div>
...
<!--                                        <SAQPreferences class="mt-4" :parent-preferences="form.preferences" v-on:modifyPreferences="updatePreferences" />-->
                                    ...
                                    <div class="form-control-errors">
                                        <p v-if="$v.form.options.$error && !$v.form.options.required" role="alert" class="text-xs text-red-500 pt-2">- {{ __('Options should not be empty') }}</p>
                                    </div>
                                    <div class="form-control-errors">
                                        <p v-if="$v.form.correct_answer.$error && !$v.form.correct_answer.required" role="alert" class="text-xs text-red-500 pt-2">- {{ __('Please specify correct answer') }}</p>
                                    </div>
                                </div>
                                <!-- Submit Button -->
                                <div class="w-full flex justify-end my-8">
                                    <button type="submit" class="qt-btn qt-btn-success" v-html="editFlag ? __('Update Details') : __('Save Details')"></button>
                                </div>
                            </form>
                        </div>
                    </div>
