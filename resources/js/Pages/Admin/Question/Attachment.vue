        ...
        <div class="container mx-auto pt-4 px-4 sm:px-6 lg:px-8">
            <div class="w-full bg-white dark:bg-gray-800 py-5 flex flex-col xl:flex-row items-start xl:items-center justify-between px-5 xl:px-10 shadow rounded-t">
                <div class="mb-4 sm:mb-0 md:mb-0 lg:mb-0 xl:mb-0 lg:w-1/2">
                    <h2 class="text-gray-800 dark:text-gray-100 text-lg font-bold">{{ __('Question') }} {{ __('Attachment') }}</h2>
                            ...
                            <form @submit.prevent="submitForm">
                                <div class="w-full flex flex-col mb-6">
                                    <div class="flex gap-2 mb-2">
                                        <label for="has_attachment" class="font-semibold text-sm text-gray-800 pb-1">{{ __('Enable Question Attachment') }}</label>
                                    </div>
                                    <SelectButton id="has_attachment" v-model="form.has_attachment" :options="choices"
                                                  dataKey="code" optionValue="code" optionLabel="name" />
                                </div>
                                <div class="w-full flex flex-col mb-6">
                                    <label class="pb-2 text-sm font-semibold text-gray-800">{{ __('Attachment Type') }}</label>
                                ...
                                <div v-if="form.has_attachment && form.attachment_type === 'comprehension'" class="form-control-errors">
                                    <p v-if="$v.form.comprehension_id.$error && !$v.form.comprehension_id.required" role="alert" class="text-xs text-red-500 pt-2">{{ __('Comprehension') }} {{ __('is required') }}.</p>
                                </div>
                                <div v-if="form.has_attachment && form.attachment_type === 'video'" class="form-control-errors">
                                    <p v-if="$v.form.attachment_options.link.$error && !$v.form.attachment_options.link.required" role="alert" class="text-xs text-red-500 pt-2">{{ __('Video Link') }} {{ __('is required') }}.</p>
                                </div>
                                <div v-if="form.has_attachment && form.attachment_type === 'audio'" class="form-control-errors">
                                    <p v-if="$v.form.attachment_options.link.$error && !$v.form.attachment_options.link.required" role="alert" class="text-xs text-red-500 pt-2">{{ __('Audio Link') }} {{ __('is required') }}.</p>
                                </div>
                                <!-- Submit Button -->
    ...
    export default {
        ...
        data() {
            return {
                ...
                attachmentTypes: [
                    { id: 'comprehension', name: this.__('Comprehension Passage')},
                    { id: 'audio', name: this.__('Audio')},
                    { id: 'video', name: this.__('Video')},
                ],
