        ...
        <div class="container mx-auto pt-8 px-4 sm:px-6 lg:px-8">
            <div class="w-full bg-white dark:bg-gray-800 py-5 flex flex-col xl:flex-row items-start xl:items-center justify-between px-5 xl:px-10 shadow rounded-t">
                <div class="mb-4 sm:mb-0 md:mb-0 lg:mb-0 xl:mb-0 lg:w-1/2">
                    <h2 class="text-gray-800 dark:text-gray-100 text-lg font-bold">{{ __('Exam') }} {{ __('Details') }}</h2>
                        ...
                        <div class="md:w-8/12 w-full py-6 md:pb-0 md:px-6">
                            <form @submit.prevent="submitForm">
                                ...
                                <div class="grid sm:grid-cols-2 sm:gap-4">
                                    ...
                                    <div class="w-full flex flex-col mb-6">
                                        <label class="pb-2 text-sm font-semibold text-gray-800">{{ __('Exam Type') }}</label>
                                        <v-select id="exam_type_id" v-model="form.exam_type_id" :options="examTypes"
                                        ...
                                        <div class="form-control-errors">
                                            <p v-if="$v.form.exam_type_id.$error && !$v.form.exam_type_id.required"
                                               role="alert" class="text-xs text-red-500 pt-2">{{ __('Exam Type') }} {{ __('is required') }}</p>
                                        </div>
                                    </div>
                                </div>
                                <!--<div class="w-full flex flex-col mb-6">
                                ...
                                <div v-if="form.is_paid" class="w-full">
                                    <div class="flex justify-between items-center mb-8">
                                        <div class="w-9/12">
                                            ...
                                            <p class="text-gray-500 text-sm">Yes ({{ __('User should redeem with points to access exam') }}). No ({{ __('Anyone can access') }}).</p>
                                        </div>
                                        <div class="cursor-pointer rounded-full relative shadow-sm">
    ...
    export default {
        ...
        data () {
            return {
                ...
                examModes: [
                    {name: this.__('Objective'), code: 'objective'},
                    {name: this.__('Subjective'), code: 'subjective'},
                    {name: this.__('Mixed (Objective & Subjective)'), code: 'mixed'},
                ],
        ...
        computed: {
            title() {
                let str = this.editFlag ? this.exam.title + ' ' + this.__('Details') : this.__('Create Exam');
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('exams.update', { id: this.examId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
