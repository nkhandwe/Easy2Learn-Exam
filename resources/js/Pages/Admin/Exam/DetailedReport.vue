<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">{{ exam.title }} - {{ schedule !== null ? 'Schedule' : '' }} {{ __('Detailed Report') }}</h4>
            <div v-if="schedule !== null" class="text-xs">{{ __('Schedule') }} ID: {{ schedule.code }}</div>
        </template>
        <template #actions>
            <a v-if="schedule !== null" :href="route('exams.export_report', {exam: exam.id, schedule: schedule.id})" target="_blank" class="qt-btn qt-btn-primary">
                {{ __('Download Report') }}
            </a>
            <a v-else :href="route('exams.export_report', {exam: exam.id})" target="_blank" class="qt-btn qt-btn-primary">
                {{ __('Download Report') }}
            </a>
            <inertia-link v-if="schedule !== null" :href="route('exams.overall_report', {exam: exam.id, schedule: schedule.id})" class="ltr:ml-2 rtl:mr-2 qt-btn qt-btn-success">
                {{ __('Overall Report') }}
            </inertia-link>
            <inertia-link v-else :href="route('exams.overall_report', {exam: exam.id})" class="ltr:ml-2 rtl:mr-2 qt-btn qt-btn-success">
                {{ __('Overall Report') }}
            </inertia-link>
        </template>
                ...
                <div class="card">
                    <div class="card-body">
                        <vue-good-table mode="remote" @on-page-change="onPageChange" @on-column-filter="onColumnFilter" @on-per-page-change="onPerPageChange"
                                        ...
                                        :totalRows="examSessions.meta.pagination.total"
                                        :rows="examSessions.data" :rtl="$page.props.rtl">
                            <template slot="table-row" slot-scope="props">
                                ...
                                <div class="py-2" v-else-if="props.column.field === 'actions'">
                                    <actions-dropdown>
                                        <template #actions>
                                            <inertia-link :href="route('exam_session_results', {exam: props.row.slug, session: props.row.id})"
        ...
        methods: {
            ...
            deleteSession(id) {
                this.$confirm.require({
                    ...
                    accept: () => {
                        this.$inertia.delete(route('delete_exam_session', {exam: this.exam.slug, session: id}), {}, {
                            onSuccess: () => {
                                this.$toast.add({
                                    severity: 'info',
