<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">{{ exam.title }} - {{ schedule !== null ? __('Schedule') : '' }} {{ __('Overall Report') }}</h4>
            <div v-if="schedule !== null" class="text-xs">{{ __('Schedule') }} ID: {{ schedule.code }}</div>
        </template>
        <template #actions>
            <inertia-link v-if="schedule !== null" :href="route('exams.detailed_report', { exam: exam.id, schedule: schedule.id })" class="qt-btn qt-btn-success">
                {{ __('Detailed Report') }}
            </inertia-link>
            <inertia-link v-else :href="route('exams.detailed_report', { exam: exam.id })" class="qt-btn qt-btn-success">
                {{ __('Detailed Report') }}
            </inertia-link>
        </template>
