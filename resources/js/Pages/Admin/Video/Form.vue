                        ...
                        <div class="max-w-4xl w-full py-6 md:pb-0 md:px-6">
                            <form @submit.prevent="submitForm">
                                <div class="w-full flex flex-col mb-6">
                                    <label for="title" class="pb-2 font-semibold text-gray-800 text-sm">{{ __('Video') }} {{ __('Title') }}</label>
                                ...
                                <div class="mb-6">
                                    <VideoOptions :parentOptions="videoOptions" v-on:modifyOptions="updateVideoOptions" />
                                </div>
                                <div class="w-full flex flex-col mb-6">
                                    <label for="thumbnail" class="pb-2 font-semibold text-gray-800 text-sm">{{ __('Video') }} {{ __('Thumbnail') }}</label>
                                    <div class="p-inputgroup">
                                        <InputText ref="thumbnail" type="text" id="thumbnail" v-model="form.thumbnail" aria-describedby="thumbnail-help" />
                                        <Button @click="chooseThumbnail()" icon="pi pi-upload" class="p-button"/>
                                    </div>
                                </div>
                                <div class="w-full flex flex-col mb-6">
                                    <label for="description" class="pb-2 font-semibold text-gray-800 text-sm">{{ __('Description') }}</label>
                                    <ckeditor id="description" :editor-url="editorUrl" v-model="form.description" :config="editorConfig" :class="[$page.props.rtl ? 'cke_rtl' : 'cke_ltr', errors.question ? 'p-invalid' : '']"></ckeditor>
                                    <div class="form-control-errors">
                                        <p v-if="$v.form.description.$error && !$v.form.description.required" role="alert" class="text-xs text-red-500 pt-2">{{ __('Body') }} {{ __('is required') }}</p>
                                    </div>
                                </div>
                                <div class="w-full flex flex-col mb-6">
                                ...
                                <div class="w-full flex flex-col mb-6">
                                    <label for="duration" class="pb-2 text-sm font-semibold text-gray-800">{{ __('Watch Time') }} ({{ __('Minutes') }})</label>
                                    <InputNumber
                                        id="duration"
                                ...
                                <div class="w-full flex justify-end my-8">
                                    <Button type="submit" :label="editFlag ? __('Update Video') : __('Save Video')" />
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                videoOptions: this.editFlag ? {video_type: this.video.video_type, link: this.video.video_link} : null,
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('videos.update', { id: this.videoId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
