<template>
    <app-layout>
        ...
        <div class="py-8">
            <div v-if="exam.settings.hide_solutions" class="mb-6" v-else>
            ...
            <div v-else>
                <div class="w-full card">
                    <div class="w-full card-body lg:flex flex-wrap">
                        <div class="w-full h-full bg-white border-b border-gray-200 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 items-center pb-4">
                            <div class="w-full h-full" v-for="(section, index) in sections" :key="'exam_section_'+section.id">
                                <button v-if="section.id === sections[current_section].id" @click="jumpToSection(index)" class="w-full ease-linear transition-all duration-150 text-sm focus:outline-none flex justify-center items-center gap-2 border border-green-800 hover:border-green-600 text-white bg-green-800 hover:bg-green-600 rounded-sm py-2 px-4">
                                    <span class="w-5 h-5 ease-linear transition-all duration-150 rounded-full bg-green-100 text-green-600">{{ section.sno }}</span>
                                    <span>{{ section.name }}</span>
                                </button>
                                <button v-else @click="jumpToSection(index)" class="w-full group ease-linear transition-all duration-150 text-sm focus:outline-none flex justify-center items-center gap-2 bg-white text-green-600 border border-green-600 hover:bg-green-600 hover:text-white rounded-sm py-2 px-4">
                                ...
                                <div v-for="(question, index) in questions" :key="question.id">
                                    <div :id="question.code" v-show="current_question === index">
                                        ...
                                        <practice-question-card :key="question.card" :question="question"
                                                                :sno="index+1"
                                                                :total-questions="sectionTotalQuestions"></practice-question-card>
                                        <div class="mt-4"
