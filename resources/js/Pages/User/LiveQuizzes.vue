    ...
    <app-layout>
        <template #header>
            <div class="flex items-center">
                <back-button />
                <h1 class="app-heading">{{ __('Live Quizzes') }}</h1>
            </div>
        </template>
            ...
            <div class="flex flex-col items-center">
                <div class="w-full">
                    <section-header :title="'Live Quizzes'">
                        ...
                        <template #action>
                            <span v-if="!loading && pagination" class="text-sm text-gray-500">{{ pagination.total }} {{ __('Schedules') }} {{ __('Found') }}</span>
                        </template>
                    </section-header>
                    ...
                    <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        <template v-for="(quiz, index) in schedules">
                            <quiz-schedule-card :quiz="quiz">
                                <template #action>
                                    <inertia-link v-if="quiz.paid && (!subscription && quiz.redeem)" :href="route('quiz_schedule_instructions', {quiz: quiz.slug, schedule: quiz.code})">
                                    ...
                                    <inertia-link v-else :href="route('quiz_schedule_instructions', {quiz: quiz.slug, schedule: quiz.code})">
