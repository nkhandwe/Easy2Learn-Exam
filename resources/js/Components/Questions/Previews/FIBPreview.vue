<template>
    <div class="bg-white shadow px-4 py-5 border-b-4 border-gray-800 mb-6">
        ...
        <div v-show="!collapse" :id="question.code+'_options'">
            <ul class="q-option" v-for="(answer, index) in question.correct_answer">
                <li class="flex items-center mb-3 cursor-pointer p-3 rounded-sm border border-gray-200">
                    <div class="h-6 w-6 text-sm flex rounded-full items-center justify-center bg-gray-100 text-sm">{{ index+1 }}</div>
                    <div class="text-sm font-normal ltr:ml-4 rtl:mr-4" v-html="answer"></div>
                </li>
            </ul>
        </div>
        <a @click="collapse = !collapse" class="qt-link-success text-sm mb-4" v-html="collapse ? __('View Answers') : __('Hide Answers')"></a>
        <ul class="mt-5">
