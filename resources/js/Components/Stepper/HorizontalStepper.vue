    ...
    <div class="w-full">
        ...
        <div class="xl:w-full xl:mx-0 h-12 hidden sm:block bg-white">
            <ul class="flex justify-end gap-8">
                <li v-for="(step, index) in steps" :key="step.key" :class="step.status === 'active' ? 'border-primary' : 'border-gray-400'"
                    ...
                    <inertia-link v-if="editFlag" :href="step.url" class="block">
                        <div class="flex items-center px-8 mb-2">
                            <div :class="step.status === 'active' ? 'bg-primary' : 'bg-gray-400'" class="w-6 h-6 group-hover:bg-primary rounded-full flex items-center justify-center">
                                <p class="focus:outline-none text-xs text-white">{{ step.step }}</p>
                            </div>
                            <span :class="step.status === 'active' ? 'text-primary' : 'text-gray-600'" class="ltr:ml-2 rtl:mr-2 group-hover:text-primary">{{ step.title }}</span>
                        </div>
                    </inertia-link>
                    <div v-else @click="showAlert" class="flex items-center px-8 mb-2">
                        <div :class="step.status === 'active' ? 'bg-primary' : 'bg-gray-300'" class="w-6 h-6 group-hover:bg-primary rounded-full flex items-center justify-center">
                            <p class="focus:outline-none text-xs text-white">{{ step.step }}</p>
                        </div>
                        <span :class="step.status === 'active' ? 'text-primary' : 'text-gray-600'" class="ltr:ml-2 rtl:mr-2 group-hover:text-primary">{{ step.title }}</span>
                    </div>
                    <div :class="step.status === 'active' ? 'bg-primary' : 'bg-gray-300'" class="w-full h-1 group-hover:bg-primary rounded-t-md"></div>
                </li>
            </ul>
        </div>
    </div>
