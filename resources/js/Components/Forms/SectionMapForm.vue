<template>
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <h3 class="mb-4 font-semibold">{{ __('Select Sections') }}</h3>
            <div v-for="section in sections" class="p-field-checkbox mb-2 py-2 cursor-pointer">
                ...
                <label class="ltr:ml-2 rtl:mr-2" :for="'section_'+section.id">{{ section.name }}</label>
            </div>
            <small id="sections-error" v-if="formValidated && selected_sections.length === 0" class="p-invalid">{{ __('Please choose at least one section') }}.</small>
            <div class="w-full flex mt-4">
                <Button type="submit" :label="__('Update')" />
            </div>
        </form>
    </div>
</template>
