    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="first_name" class="pb-2 font-semibold text-gray-800">{{ __('First Name') }}<span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="first_name"
            ...
            <!-- Last Name -->
            <div class="w-full flex flex-col mb-6">
                <label for="last_name" class="pb-2 font-semibold text-gray-800">{{ __('Last Name') }}<span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="last_name"
            ...
            <!-- Email -->
            <div class="w-full flex flex-col mb-6">
                <label for="email" class="pb-2 font-semibold text-gray-800">{{ __('Email') }}<span class="ml-1 text-red-400">*</span></label>
            ...
            <!-- User Name -->
            <div class="w-full flex flex-col mb-6">
                <label for="user_name" class="pb-2 font-semibold text-gray-800">{{ __('User Name') }}<span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="user_name"
            ...
            <!-- User Role Dropdown -->
            <div class="w-full flex flex-col mb-6">
                <label for="userRole" class="pb-2 font-semibold text-gray-800">{{ __('User Role') }}<span class="ml-1 text-red-400">*</span></label>
                <v-select id="userRole" v-model="form.role" :options="roles" :reduce="role => role.value" label="text" placeholder="Choose Role" :dir="$page.props.rtl ? 'rtl' : 'ltr'">
            ...
            <!-- Password -->
            <div class="w-full flex flex-col mb-6">
                <label for="password" class="pb-2 font-semibold text-gray-800">{{ __('Password') }}<span v-if="!editFlag" class="ml-1 text-red-400">*</span></label>
            ...
            <div class="w-full">
                <div class="flex justify-between items-center mb-8">
                    <div class="w-9/12">
                        <label for="email_verified_at" class="font-semibold text-gray-800 pb-1" v-html="form.email_verified_at ? __('Email Verified')+' - '+__('Yes') : __('Email Verified')+' - '+ __('No')"></label>
                        <p class="text-gray-500">{{ __('Yes') }} ({{ __('Email is verified') }}). {{ __('No') }} ({{ __('Email not verified') }}).</p>
                    </div>
                    <div class="cursor-pointer rounded-full relative shadow-sm">
                        <InputSwitch id="email_verified_at" v-model="form.email_verified_at" />
                    </div>
                </div>
            </div>
            ...
            <div class="w-full">
                <div class="flex justify-between items-center mb-8">
                    <div class="w-9/12">
                        ...
                        <p class="text-gray-500">{{ __('Active') }} ({{ __('Allow Login') }}). {{ __('In-active') }} ({{ __('Disallow Login') }}).</p>
                    </div>
                    <div class="cursor-pointer rounded-full relative shadow-sm">
