    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <div class="w-full flex flex-col mb-6">
                <label for="name" class="pb-2 font-semibold text-gray-800">{{ __('Exam Type') }} {{ __('Name') }}</label>
                <InputText type="text"
                           id="name"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('exam-types.update', { id: this.examTypeId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('exam-types.edit', { id: this.examTypeId }))
