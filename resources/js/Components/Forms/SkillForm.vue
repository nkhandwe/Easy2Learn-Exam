    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <div class="w-full flex flex-col mb-6">
                <label for="name" class="pb-2 font-semibold text-gray-800">{{ __('Concept') }} {{ __('Name') }}</label>
                <InputText type="text"
                           id="name"
                           v-model="form.name"
                           placeholder="Enter Concept Name" aria-describedby="name-help"
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="section" class="pb-2 font-semibold text-gray-800">{{ __('Section') }}<span class="ml-1 text-red-400">*</span></label>
                <v-select id="section" v-model="form.section_id" :options="sections" :reduce="sec => sec.id" label="name" :dir="$page.props.rtl ? 'rtl' : 'ltr'">
                ...
                <small id="section-help" v-if="errors.section_id" class="p-invalid">{{ errors.section_id }}</small>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('skills.update', { id: this.skillId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('skills.edit', { id: this.skillId }))
