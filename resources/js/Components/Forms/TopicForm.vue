    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <div class="w-full flex flex-col mb-6">
                <label for="name" class="pb-2 font-semibold text-gray-800">{{ __('Topic') }} {{ __('Name') }}</label>
                <InputText type="text"
                           id="name"
                           v-model="form.name"
                           :placeholder="__('Topic')" aria-describedby="name-help"
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="skill" class="pb-2 font-semibold text-gray-800">{{ __('Concept') }}<span class="ml-1 text-red-400">*</span></label>
                ...
                <small id="skill-help" v-if="errors.skill_id" class="p-invalid">{{ errors.skill_id }}</small>
            </div>
            <div class="w-full flex flex-col mb-6">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('topics.update', { id: this.topicId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('topics.edit', { id: this.topicId }))
