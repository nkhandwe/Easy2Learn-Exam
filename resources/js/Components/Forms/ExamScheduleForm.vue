    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            <div class="w-full flex flex-col mb-6">
                <label class="pb-2 text-sm font-semibold text-gray-800">{{ __('Schedule Type') }}<span class="ml-1 text-red-400">*</span></label>
            ...
            <div v-if="form.schedule_type === 'fixed'" class="w-full flex flex-col mb-6">
                <label for="grace_period" class="pb-2 font-semibold text-gray-800">{{ __('Grace Period to Join') }} ({{ __('Minutes') }})</label>
            ...
            <div v-if="editFlag" class="w-full flex flex-col mb-6">
                ...
                <Dropdown id="status" v-model="form.status" :options="examStatus" optionLabel="name" optionValue="code" dataKey="code"
            ...
            <small id="expire-note" v-if="disableFlag" class="text-sm text-red-400">{{ __('schedule_update_message') }}</small>
    ...
    export default {
        ...
        data() {
            return {
                ...
                disableFlag: false,
                scheduleTypes: [
                    {name: this.__('Fixed'), code: 'fixed'},
                    {name: this.__('Flexible'), code: 'flexible'},
                ],
                examStatus: [
                    {name: this.__('Active'), code: 'active'},
                    {name: this.__('Expired'), code: 'expired'},
                    {name: this.__('Cancelled'), code: 'cancelled'},
                ],
        ...
        methods: {
            ...
            create() {
                ...
                this.$inertia.post(route('exams.schedules.store', { exam: this.examId }), this.form, {
                    onSuccess: () => {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('exams.schedules.update', { exam: this.examId, schedule: this.examScheduleId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('exams.schedules.edit', { exam: this.examId, schedule: this.examScheduleId }))
